...
impl WsChatServer {
    ...
    fn send_chat_message(
        ...
        for (_client_id, client) in room.iter() {
            client.recipient.do_send(ChatMessage(msg.to_owned())).ok()?;
        }
...
impl Handler<ListClients> for WsChatServer {
    ...
    fn handle(&mut self, msg: ListClients, _ctx: &mut Self::Context) -> Self::Result {
        ...
        if let Some(room) = self.rooms.get(room_name.as_str()) {
            let mut client_names: Vec<String> = room
                .values()
                .map(|client_name| format!("{}", client_name.name))
                .collect();
            ...
            client_names.insert(0, String::from("clients-list"));
...
impl Handler<SetName> for WsChatServer {
    type Result = ();

    fn handle(&mut self, msg: SetName, _ctx: &mut Self::Context) -> Self::Result {
        let SetName(room_name, client_id, client_name) = msg;

        if let Some(room) = self.rooms.get_mut(room_name.as_str()) {
            if let Some(mut client) = room.get_mut(&client_id) {
                client.name = client_name;
            }
        }

        let join_msg = format!("New client");

        self.send_chat_message(&room_name, &join_msg, client_id);
