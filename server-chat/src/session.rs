...
use crate::message::{
    ChatMessage, JoinRoom, LeaveRoom, ListClients, ListRooms, SendMessage, SetName
};
...
impl WsChatSession {
    ...
    pub fn join_room(&mut self, room_name: &str, ctx: &mut ws::WebsocketContext<Self>) {
        ...
        let join_msg = JoinRoom(
            ...
            Client {
                ...
                name : self.client_name.clone().unwrap_or(String::from("anon")),
            }
        );
            ...
            .then(|result, _, ctx| {
                if let Ok(clients) = result {
                    ...
                    for client in clients {
                        if list == String::from("") {
                            list.push_str(&client);
                        } else {
                            let add_client = ",".to_owned() + &client;
                            list.push_str(&add_client);
                        }
                    }
                    ctx.text(list);
                }
    ...
    pub fn send_msg(&self, msg: &str) {
        let content = format!("Message,{},{}", self.client_name(), msg);
    ...
    pub fn set_name(&mut self, client_name: String, ctx: &mut ws::WebsocketContext<Self>) {
        let set_name_msg = SetName(
    ...
    fn handle(
        ...
        match msg {
            ws::Message::Text(text) => {
                let msg = text.trim();
                let data_message = msg.replace("\"", "");
                println!("WEBSOCKET MESSAGE: {:?}", data_message);

                if data_message.starts_with('/') {
                    println!("GET THE COMMAND");
                    let mut command = data_message.splitn(2, ' ');
                    ...
                    match command.next() {
                        ...
                        Some("/name") => {
                            if let Some(name) = command.next() {
                                ...
                                self.set_name(name.to_owned(), ctx);
